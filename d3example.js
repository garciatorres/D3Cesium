...
// array of the attribute options
    ...
    function dot_value(a,d) {
      if (a==attribute_options[0])  return d.__data__.type;
      if (a==attribute_options[1])  return d.__data__.charge;
      if (a==attribute_options[2])  return d.__data__.arrest;
      if (a==attribute_options[3])  return d.__data__.article;
      if (a==attribute_options[4])  return d.__data__.agency;
      if (a==attribute_options[5])  return d.__data__.accident;
      if (a==attribute_options[6])  return d.__data__.belt;
      if (a==attribute_options[7])  return d.__data__.injury;
      if (a==attribute_options[8])  return d.__data__.damage;
      if (a==attribute_options[9])  return d.__data__.fatal;
      if (a==attribute_options[10]) return d.__data__.hazmat;
      if (a==attribute_options[11]) return d.__data__.alcohol;
      if (a==attribute_options[12]) return d.__data__.vehicle;
      if (a==attribute_options[13]) return d.__data__.make;
      if (a==attribute_options[14]) return d.__data__.race;
      if (a==attribute_options[15]) return d.__data__.gender;
    }
    ...
    var h_margin = {top: 19.5, right: 19.5, bottom: 19.5, left: 39.5},
        h_width = 960 - h_margin.left - h_margin.right,
        h_height = 500 - h_margin.top - h_margin.bottom;
...
/********************************************************* Dot drawing */
      ...
      dot.append("title").text(function(d) {
        return "ID:"+d.id+" Date:"+d.date.substring(0,10)+" "+
        "Latitude:"+y(d).substring(0,5)+" "+"Longitude:"+x(d).substring(0,5) ; })
      $(".dot").tipsy({ gravity: 's', });

      var bars;

/********************************************************* Histogram drawing */
      var frequency = function(f) {
        var c = 0;
          dot.forEach(function (s) {
            s.forEach(function (d) {
            if (d.__data__.show == true && dot_value(sharedObject.showBy,d)==f)
                c = c+1;
          })});
        return c;
      }

      var build_histogram = function() {
        var histogram = [];
        // Builds histogram's data structure
        distinct_values.forEach(function (f) {
          var tuple = { key: f, value: frequency(f) };
          histogram.push(tuple);
        });

        //set up svg using margin conventions - we'll need plenty of room on the left for labels
        var h_margin = { top: 15, right: width, bottom: 15, left: 25 };

        var h_width = 500 - h_margin.left - h_margin.right,
            h_height = 500 - h_margin.top - h_margin.bottom;

        var h_x = d3.scale.linear().range([0, violations.data.length])
        .domain([0, violations.data.length]);

        var h_y = d3.scale.ordinal().rangeRoundBands([h_height, 0], .1)
        .domain(histogram.map(function (d) { return d.key;}));

        //make y axis to show bar names
        var h_yAxis = d3.svg.axis().scale(h_y).tickSize(0).orient("left").tickValues([]);
        var gy = svg.append("g").attr("class", "y axis").call(h_yAxis);

        if (typeof bars !='undefined') bars.selectAll(".bar").remove();
        bars = svg.selectAll(".bar").data(histogram).enter().append("g");

        //append rects
        bars.append("rect").attr("class", "bar")
        .attr("y", function (d) {return h_y(d.key);})
        .attr("height", h_y.rangeBand())
        .attr("x", width + h_margin.left )
        .attr("width", function (d) {return h_x(d.value);})
        .attr("fill", function(d) { return colorScale(d.key);});

        /*add a value label to the right of each bar
        bars.append("text").attr("class", "label")
        //y position of the label is halfway down the bar
        .attr("y", function (d) { return h_y(d.key) + h_y.rangeBand() / 2 + 4; })
        //x position is 3 pixels to the right of the bar
        .attr("x", function (d) { return h_x(d.value) + width - 10; })
      ...
      var show_histogram = function() {
        if (sharedObject.histogram==false) {
            sharedObject.histogram= true;
            d3.select("#chart").select("svg")
            .attr("width", width + margin.left + margin.right + 500);
            build_histogram();
        } else {
            sharedObject.histogram=false;
            d3.select("#chart").select("svg")
            .attr("width", width + margin.left + margin.right);
        }
      }

      svg.selectAll("histogram_buttom")
      .data(["Histogram"])
      .enter()
      .append("text")
      .attr("x", 865)
      ...
      function displayYear(year) {
        ...
        if (sharedObject.histogram==true) build_histogram();
      }
